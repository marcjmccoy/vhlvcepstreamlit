...
def classify_vhl_ps1(test_variant):
            ...
            if ref_protein_change and ('fs' not in ref_protein_change and 'Ter' not in ref_protein_change):
                if protein_change == ref_protein_change:
                    ...
                    return {
                        'strength': 'PS1',
                        'context': (
                            f"variant {protein_change} matches previously established VHL VCEP/ClinGen pathogenic "
    ...
    # All other types return context per ACMG/AMP PS1 guideline
        ...
        return {'strength': None,
                'context': "This is a splice site variant; PS1 applies only to previously established VHL VCEP/ClinGen pathogenic variants"}
    elif vt == "frameshift":
        return {'strength': None,
                'context': "This is a frameshift variant; PS1 applies only to previously established VHL VCEP/ClinGen pathogenic variants"}
    elif vt == "nonsense":
        return {'strength': None,
                'context': "This is a nonsense/stop-gain variant; PS1 applies only to previously established VHL VCEP/ClinGen pathogenic variants"}
    elif vt in ["inframe_del", "inframe_dup", "inframe_ins"]:
        return {'strength': None,
                'context': f"This is an in-frame {vt.split('_')[1]} variant; PS1 applies only to previously established VHL VCEP/ClinGen pathogenic variants"}
    else:
        details = []
        if not protein_change:
            details.append("PS1 applies only to previously established VHL VCEP/ClinGen pathogenic variants")
